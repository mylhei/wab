...
app.controller('OrderAutoDetailCtrl', ['$scope', '$http', 'toaster', '$state', '$modal', '$log', '$rootScope', '$stateParams', 'appTools', 'OrderService','orderService','adPositionService','areaService',
    function($scope, $http, toaster, $state, $modal, $log, $rootScope, $stateParams, appTools, OrderService,orderService,adPositionService,areaService) {
        toaster.pop('error', '注意', '由于播放器限制,编辑可视订单时,播放进度可能不准,请手动播放到位置后点击暂停!');
        ...
        $scope.play = function () {
            ...
            //后台接口请求智能打点广告位信息
            adPositionService.queryAdPositions(vid.id).then(function(adps){
        ...
        orderService.getOrderById($scope.form.id).then(function (order) {
            ...
            if(typeof $scope.form.coordinate == 'string'){
                $scope.form.coordinate = JSON.parse($scope.form.coordinate);
            }
            $scope.onPlatformChanged();
