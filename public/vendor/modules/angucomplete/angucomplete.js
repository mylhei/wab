    ...
    return {
        ...
        link: function($scope, elem, attrs) {
            ...
            $scope.processResults = function(responseData, str) {
                if (responseData && responseData.length > 0) {
                    ...
                    for (var i = 0; i < responseData.length; i++) {
                        ...
                        if(titleCode[0] == titleCode[1]){
                            titleCode.splice(1,1);
                        }
            ...
            $scope.selectResult = function(result) {
                ...
                $scope.showDropdown = false;
                $scope.$emit('selectResultDone',result);
                $scope.results = [];
                //$scope.$apply();
            }
            ...
            $scope.$watch('searchStr', function (newV, oldV) {
                if (newV == "" && newV != oldV) {
                    $scope.$emit('selectResultNone', newV, $scope.results);
                }
            });

            $scope.$on('search_type_changed',function(evt,value){
                if (value){
                    $scope.searchStr=null;
                    $scope.url = value.url;
                    $scope.placeholder = value.text;
                    $scope.$emit('selectResultNone', '', []);
                }
            });
