(function () {
    ...
    var DatetimePickerCtrl = function ($compile, $document) {
        ...
        this.openDatetimePicker = function (options) {
            ...
            options.second && div.attr('second', parseInt(options.second));
    ...
    var tmpl = [
        ...
        '  </div>',
        '  <div class="adp-days col-sm-3" ng-click="setDate($event)">',
        ...
        '  </div>',
        '  <div class="adp-days col-sm-3" id="adp-time"> ',
        '    <label class="timeLabel">时间:</label> <span class="timeValue">{{("0"+inputHour).slice(-2)}} : {{("0"+inputMinute).slice(-2)}} : {{("0"+inputSecond).slice(-2)}}</span><br/>',
        '    <label class="hourLabel">小时:</label> <input class="hourInput" type="range" min="0" max="23" ng-model="inputHour" ng-change="updateNgModel()" />',
        '    <label class="minutesLabel">分:</label> <input class="minutesInput" type="range" min="0" max="59" ng-model="inputMinute"  ng-change="updateNgModel()"/> ',
        '    <label class="secondsLabel">秒:</label> <input class="secondsInput slider slider-horizontal" type="range" min="0" max="59" ng-model="inputSecond"  ng-change="updateNgModel()"/> ',
        '  </div> ',
        '</div>'].join("\n");
        ...
        return {
            ...
            link: function (scope, element, attrs, ctrl) {
                ...
                scope.$watch(attrs.ngModel, function (value) {
                    ...
                    ctrl.$setValidity('date', date.toString() == "Invalid Date" ? false : true);
                    ctrl.$setDirty('date', date.toString() == "Invalid Date" ? false : true);
                    var now = new Date();
                    if (attrs.hasOwnProperty('futureOnly')) {
                        ctrl.$setDirty('future-only', date < now ? false : true);
